...
protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
	 ...
	 // -> 반환 형태가 String 배열 형태
	 // -> 제출된 값 중 name 속성 값이 일치하는 것을 모두 모아 하나의 String[]배열로 반환
	 ...
	 sb.append(orderName + "님의 주문영수증");
	 // 주문하는 사람에 따라서 페이지 제목 바꾸기
	 ...
	 // 삼항 연산자, bone이 true면 뼈 출력 / 아니면 순살 출력
	 
	 // 옵션이 있을 경우
	 if(options != null) {
		 sb.append("<li>선택한 옵션 : ");
		 // String.join() : String[] 배열을 -> String 한 줄로 변환
		 sb.append(String.join(" / ", options));
		 // 치킨 무 / 콜라 / 치즈볼 ( 슬래시 / 는 구분자)
		 sb.append("</li>");
	 }
