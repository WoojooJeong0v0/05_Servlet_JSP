...
<body>
  ...
  <%-- JSP 주석 -> 자바 주석으로 변환 예정
    -> 랜더링 된 HTML에서 보이지 않는 주석

    언제 사용? : JSP에 작성된 Java코드를 주석 처리 할 때
   --%>
   ...
   <%-- 
    ...
    - 변수 선언, if, for 등 모두 작성 가능하다
    - JSP -> Servlet 변환 시 작성된 형태 그대로 Servlet 중간에 작성됨

    <%= %> : Expression (표현식) JSP Expression
  ...
  <%
    ...
    // 비밀번호가 일치하는 경우
    if(pw.equals(check)){ %>
    <h1 style = "color : skyblue;"> 가입 성공</h1>
  <% } else { %>
    <h1 style = "color : pink;"> 비밀번호 불일치 </h1>
  <% } %>
</body>
</html>